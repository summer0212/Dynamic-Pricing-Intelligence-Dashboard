...
## 12. Mock Pricing Engine — How It Works

The mock engine in `services/pricing_engine.py` simulates what the real AI agent will do. For each product:

```mermaid
flowchart TD
    A["Input: Product with current_price, cost_price, inventory_count, category"] --> B{"Inventory Level?"}

    B -->|"Less than 50 units - Low Stock"| C["Direction: INCREASE<br/>Range: +5% to +15%<br/>Signal: Scarcity pricing"]
    B -->|"More than 250 units - High Stock"| D["Direction: DECREASE<br/>Range: -3% to -10%<br/>Signal: Clearance pricing"]
    B -->|"50 to 250 units - Normal"| E["Direction: RANDOM<br/>Range: plus or minus 2% to 8%<br/>Signal: Competitor adjustment"]

    C --> F{"Recommended price above cost x 1.05?"}
    D --> F
    E --> F

    F -->|Yes| G["Keep recommended price"]
    F -->|No| H["Floor at cost x 1.05<br/>ensure 5% minimum margin"]

    G --> I["Output:<br/>recommended_price as Decimal<br/>confidence_score 0.60 to 0.95<br/>rationale as text<br/>agent_outputs as JSON"]
    H --> I
```

> [!IMPORTANT]
> **This is the only file to replace** when implementing the real LangChain/Groq AI agent. The function signature (Product in, dict out) and return keys stay the same, so the routes and frontend don't change.
